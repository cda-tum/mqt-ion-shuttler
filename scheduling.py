...
def preprocess(memorygrid, sequence):
    ...
    rotated = [False] * len(sequence)
    while sum(rotated) < len(sequence):
        for i, rotate_chain in enumerate(sequence):
            edge_idc = memorygrid.ion_chains[rotate_chain]
            # fix pz for now
            pz = memorygrid.pzgraph_creator.pzs[0]
            next_edge = memorygrid.find_next_edge(edge_idc, pz)
...
def create_move_list(memorygrid, sequence, max_length=10):
    ...
    # for exit_connection_idc in memorygrid.pzgraph_creator.path_to_pz:
    ...
    for ion, chain_edge_idx in enumerate(memorygrid.get_state_idxs()):
        if chain_edge_idx in pz.path_from_pz_idxs:
            if chain_edge_idx == get_idx_from_idc(memorygrid.idc_dict, pz.entry_edge):
    ...
    # # fix pz for now
    # pz = memorygrid.pzgraph_creator.pzs[0]
    # move_list = [(ion, pz) for ion in move_list]
...
def create_circles_for_moves(memorygrid, move_list, flat_seq, gate_execution_finished, new_gate_starting):
    ...
    rotate_entries = {pz: (False, None) for pz in memorygrid.pzgraph_creator.pzs}
    parking_open = {pz: True for pz in memorygrid.pzgraph_creator.pzs}
    chain_to_park = {pz: None for pz in memorygrid.pzgraph_creator.pzs}

    for pz in memorygrid.pzgraph_creator.pzs:
        chain_to_park[pz] = memorygrid.find_chain_in_edge(pz.path_to_pz[-1])
        chain_to_move_out_of_pz = None
        if memorygrid.count_chains_in_parking(pz) < memorygrid.max_num_parking or gate_execution_finished[pz]:
            parking_open[pz] = True
        else:
            parking_open[pz] = False
    ...
    for rotate_chain in move_list:
        ...
        next_edges[rotate_chain] = memorygrid.find_next_edge(edge_idc, pz, towards=towards)
    ...
    for rotate_chain in move_list:
        ...
        # moves in pz
        for pz in memorygrid.pzgraph_creator.pzs:
            pz.in_and_into_exit_moves = {}
            if get_idx_from_idc(memorygrid.idc_dict, next_edge) in [
                *pz.path_to_pz_idxs,
                get_idx_from_idc(memorygrid.idc_dict, pz.parking_edge),
            ]:
                pz.in_and_into_exit_moves[rotate_chain] = edge_idc
            ...
            # also if chain is moving out of entry connections (entry is handled in create_outer_circle)
                ...
                or get_idx_from_idc(memorygrid.idc_dict, edge_idc) in pz.path_from_pz_idxs[:-1]
            ):
                all_circles[rotate_chain] = [edge_idc, next_edge]
    ...
    for pz in memorygrid.pzgraph_creator.pzs:
        ...
        chains_in_parking = memorygrid.find_chains_in_parking(pz)

        # if pz full and no chain is moving out (not in state_idxs entry edge) but chain is moving in
        if memorygrid.count_chains_in_parking(pz) >= memorygrid.max_num_parking and chain_to_park[pz] is not None:
            # if gate finished -> new space could be in parking
            if gate_execution_finished[pz]:
                # find least important chain in parking edge
                chain_to_move_out_of_pz = memorygrid.find_least_import_chain_in_parking(
                    flat_seq, [*chains_in_parking, chain_to_park[pz]]
                )
                if chain_to_move_out_of_pz != chain_to_park[pz]:
                    # move it to entry
                    rotate_entries[pz] = (True, chain_to_move_out_of_pz)
    ...
    return all_circles, rotate_entries
...
def rotate_free_circles(memorygrid, all_circles, free_circle_seq_idxs, rotate_entries):
    # ROTATE CIRCLES #
    ...
    for pz, (rotate_entry, chain_to_move_out_of_pz) in rotate_entries.items():
        if rotate_entry:
            memorygrid.ion_chains[chain_to_move_out_of_pz] = pz.path_from_pz[0]
...
def update_sequence_and_process_gate(
    ...
    gate = seq[0]
    chains_in_parking = {pz: memorygrid.find_chains_in_parking(pz) for pz in memorygrid.pzgraph_creator.pzs}
    ...
    for pz in memorygrid.pzgraph_creator.pzs:
        ...
        # if gate can be executed in pz
        if sum((gate_element in chains_in_parking[pz]) for gate_element in gate) == len(gate):
            gate_execution_finished[pz] = False

            time_in_pz_counter[pz] += 1
            ...
            # if gate is finished
            if time_in_pz_counter[pz] == time_gate:
                ...
                time_in_pz_counter[pz] = 0
                gate_execution_finished[pz] = True
                pzs_available[pz] = False
                ...
                for gate_element in seq[0]:
                    new_gate_starting[pz] = gate_element in chains_in_parking
...
def run_simulation(memorygrid, max_timesteps, seq, flat_seq, dag_dep, next_node_initial, max_length):
    time_in_pz_counter = {pz: 0 for pz in memorygrid.pzgraph_creator.pzs}
    next_gate_is_two_qubit_gate = len(seq[0]) == 2
    gate_execution_finished = {pz: True for pz in memorygrid.pzgraph_creator.pzs}
    new_gate_starting = {pz: False for pz in memorygrid.pzgraph_creator.pzs}
    timestep = 0
    ...
    while timestep < max_timesteps:
        ...
        # TODO check_duplicates(state_idxs, memorygrid, memorygrid.pzgraph_creator.parking_edge, memorygrid.max_num_parking)
        ...
        all_circles, rotate_entries = create_circles_for_moves(
        ...
        # rotate free circles
        rotate_free_circles(memorygrid, all_circles, free_circle_seq_idxs, rotate_entries)
        # now parallel gates in multiple pzs
        pzs_available = {pz: True for pz in memorygrid.pzgraph_creator.pzs}
        while sum(pzs_available.values()) > 0:
            ...
            if finished:
                return timestep
            timestep += 1
            print('pzs_available', [(pz_key.name, pz_val) for (pz_key, pz_val) in pzs_available.items()])
            print('gate_execution_finished', [(pz_key.name, pz_val) for (pz_key, pz_val) in gate_execution_finished.items()])
            print('new_gate_starting', [(pz_key.name, pz_val) for (pz_key, pz_val) in new_gate_starting.items()])
            # break if no gate can be executed (here still single first gate, not front layer)
            if sum(new_gate_starting) == 0:
                break
...
#   -> it should use the whole front layer of gates 
#   -> right now first gate of front layer is check if executable 
#   -> if not, others are skipped even though they might be possible in other pzs