...
def create_graph(m, n, ion_chain_size_vertical, ion_chain_size_horizontal):
    ...
    # already done above (bottom pz)
    # exit1 = (float((m-1)*ion_chain_size_vertical), float((n-1)*ion_chain_size_horizontal))
    # entry1 = (float((m-1)*ion_chain_size_vertical), float(0))
    # processing_zone1 = (float((m-1)*ion_chain_size_vertical-height), float((n-1)*ion_chain_size_horizontal/2))

    exit2 = (0.0, 0.0)
    entry2 = (0.0, float((n-1)*ion_chain_size_horizontal))
    processing_zone2 = (float(height), float((n-1)*ion_chain_size_horizontal/2))

    exit3 = (float((m-1)*ion_chain_size_vertical), float(0))
    entry3 = (float(0), float(0))
    processing_zone3 = (float((m-1)*ion_chain_size_vertical/2), float(height))

    exit4 = (float(0), float((n-1)*ion_chain_size_horizontal))
    entry4 = (float((m-1)*ion_chain_size_vertical), float((n-1)*ion_chain_size_horizontal))
    processing_zone4 = (float((m-1)*ion_chain_size_vertical/2), float((n-1)*ion_chain_size_horizontal-height))
    
    # top
    #parking_node = (processing_zone2[0] - 1, processing_zone2[1])  # Above processing zone
    networkx_graph.add_node(processing_zone2, node_type="processing_zone_node", color="r")
    networkx_graph.add_edge(exit2, processing_zone2, edge_type="exit", color="k")
    networkx_graph.add_edge(
        processing_zone2, entry2, edge_type="entry", color="r"
    )

    # left
    #parking_node = (processing_zone3[0], processing_zone3[1] - 1)  # Left of processing zone
    networkx_graph.add_node(processing_zone3, node_type="processing_zone_node", color="r")
    networkx_graph.add_edge(exit3, processing_zone3, edge_type="exit", color="k")
    networkx_graph.add_edge(
        processing_zone3, entry3, edge_type="entry", color="r"
    )
    # right
    #parking_node = (processing_zone4[0], processing_zone4[1] + 1)  # Right of processing zone
    networkx_graph.add_node(processing_zone4, node_type="processing_zone_node", color="r")
    networkx_graph.add_edge(exit4, processing_zone4, edge_type="exit", color="k")
    networkx_graph.add_edge(
        processing_zone4, entry4, edge_type="entry", color="r"
    )

    plot_state(networkx_graph, plot_ions=False)
    plt.show()
...
class MemorySAT:
    ...
    def create_constraints(self, starting_traps):
        ...
        for t in range(1, self.timesteps):
            for edge_idc in self.graph.edges():
                edge_idc = get_idc_from_idx(self.idc_dict, get_idx_from_idc(self.idc_dict, edge_idc))
                if nx.get_edge_attributes(self.graph, "edge_type")[edge_idc] == "entry":
                    self.s.add(
                        AtMost(
